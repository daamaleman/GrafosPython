...
class Grafo:
    ...
    def agregar_vertice(self, vertice):
        ...
        lo añade con un conjunto vacío de vecionos."""
    ...
    def agregar_arista(self, u, v, peso=1):
        # Asegurarse de que ambos vertices existan en el grafo
        self.agregar_vertice(u)
        self.agregar_vertice(v)
        
        # Anadir la arista
        self.grafo[u].add(v)
        print(f"Arista {u} -> {u} agregada.")
        
        # Si no es dirigido, anadir la arista en la direccion opuesta tambien 
        if not self.es_dirigido:
            self.grafo[v].add(u)
            print(f"Arista {v} -> {u} (bidirrecionales) agregada.")
    ...
    def existe_arista(self, u, v):
        # Verifica si ambos vertices y si 'v' esta en la lista de adyacencia de 'u'
    ...
    def bfs(self, inicio):
        ...
        while cola:
            ...
            # Anadir a la cola los vecionos no visitados
    ...
    def encontrar_camino(self, inicio, fin):
        ...
        visitados = set()
        padres = {} # Para reconstruit el camino: padres[hijo] = padre
...
print(f"¿Existe arista Managua -> Masaya?: {mi_grafo.existe_arista('Managua', 'Masaya')}")
print(f"¿Existe arista Managua -> Rivas?: {mi_grafo.existe_arista('Managua', 'Rivas')}")
...
print(f"¿El grafo es conexo?: {mi_grafo.es_conexo()}")
...
print(f"¿Existe arista A -> B?: {grafo_dirigido.existe_arista('A', 'b')}")
print(f"¿Existe arista B -> A?: {grafo_dirigido.existe_arista('B', 'A')}")
...
print(f"Camino de Inicio a Fin (Dirigido): {camino_dirigido}")

camino_dirigido_inexistente = grafo_dirigido.encontrar_camino('Fin', 'Inicio')
print(f"Camino de Fin a Inicio (Dirigido): {camino_dirigido_inexistente}")
